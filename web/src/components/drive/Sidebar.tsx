  ...
  return (
    <aside className="flex h-full w-64 shrink-0 flex-col bg-sidebar text-sidebar-foreground">
      ...
      <div className="px-6 py-5">
        <Logo iconHeight={32} iconWidth={27} textClassName="text-lg" />
      </div>
      ...
      <nav className="flex-1 space-y-1 px-4">
        {NAV_ITEMS.map(({ label, to, icon: Icon, exact }) => {
          ...
          return (
            ...
            >
              <Icon size={18} className={isActive ? 'text-accent-foreground' : 'text-foreground/80'} />
              {label}
            </Link>
          )
        })}
      ...
      {storage && (
        <div className="px-6 py-6 border-t border-border/10">
          ...
          <button className="mt-4 rounded-full border border-border px-4 py-1.5 text-sm font-normal text-primary hover:bg-primary/10 transition-colors">
            Get more storage
          </button>
        </div>
      )}
